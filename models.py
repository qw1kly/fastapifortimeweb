...
async def game_ch(m):
    ...
    access_password = "SELECT * FROM password"
            ...
            if str(m[1]) == i[1]:
                ...
                for j in range(len(friends)):
                    if child_id in friends[j]:
                        friends[j].append(int(i[2]))
        dicta = {}
        count=0
        i = 0
        while i < len(friends):
            dicta[count] = friends[i][0]
            dicta[count+1] = friends[i][1]
            dicta[count+2] = friends[i][2]
            dicta[count+3]= 'END'
            count+=4
            i+=1
        connect.close()
...
async def make_address_payment(m):
    ...
    async with connect.cursor() as cur:
        ...
        connect.close()
        return {1:2}
# async def make_withdarawal(m):
#     async def main() -> None:
#         API_KEY = "AF6J7NK5PWFMX2IAAAABCYNPNKHXUHWIP3O4FJWRVFZVEZV6LC5BCJ5DGQQ4GNDP4XX73UQ"
#         IS_TESTNET = False

#         MNEMONIC = ['visit', 'horror', 'ice', 'spice', 'flee', 'summer', 'bless', 'version', 'credit', 'attract', 'private',
#                     'rough', 'gown', 'deputy', 'asthma', 'forest', 'exhaust', 'clarify', 'vocal', 'upgrade', 'divorce',
#                     'fog', 'reunion', 'vendor']

#         client = TonapiClient(api_key=API_KEY, is_testnet=IS_TESTNET)
#         wallet, public_key, private_key, mnemonic = WalletV4R2.from_mnemonic(client, MNEMONIC)
#         tx_hash = await wallet.transfer(
#             destination="UQD0OT6wBrdA7n_8ZUfcwFCpRrgtQL00D4nRLnfam17WKdaG",
#             amount=0.01,
#             body='hiii',
#         )
#         print(f"Transaction hash: {tx_hash}")
#         return None #hash result
#     hash_res = await asyncio.create_task(main())
